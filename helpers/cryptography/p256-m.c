 ...
 */
#define zeroize(s, size) memset(s, 0, size)
 ...
 */
#if defined(__GNUC__) && defined(__ARM_ARCH) && __ARM_ARCH >= 6 && defined(__ARM_ARCH_PROFILE) && \
...
static void u256_from_bytes(uint32_t z[8], const uint8_t p[32]) {
    for(unsigned i = 0; i < 8; i++) {
        unsigned j = 4 * (7 - i);
        z[i] = ((uint32_t)p[j + 0] << 24) | ((uint32_t)p[j + 1] << 16) |
               ((uint32_t)p[j + 2] << 8) | ((uint32_t)p[j + 3] << 0);
    }
}
 ...
 */
static void
    m256_add(uint32_t z[8], const uint32_t x[8], const uint32_t y[8], const m256_mod* mod) {
    uint32_t r[8];
 ...
 */
static void
    m256_sub(uint32_t z[8], const uint32_t x[8], const uint32_t y[8], const m256_mod* mod) {
    uint32_t r[8];
 ...
 */
static void
    m256_mul(uint32_t z[8], const uint32_t x[8], const uint32_t y[8], const m256_mod* mod) {
    /*
...
int p256_gen_keypair(uint8_t priv[32], uint8_t pub[64]) {
    ...
    zeroize(s, sizeof s);
    if(ret != 0) return P256_RANDOM_FAILED;
...
int p256_ecdsa_sign(uint8_t sig[64], const uint8_t priv[32], const uint8_t* hash, size_t hlen) {
    ...
    if(u256_diff0(xr) == 0) return P256_RANDOM_FAILED;
    ...
    if(ret != 0) return P256_INVALID_PRIVKEY;
...
int p256_ecdsa_verify(
    ...
    if(ret != 0) return P256_INVALID_SIGNATURE;
    ...
    if(ret != 0) return P256_INVALID_PUBKEY;
