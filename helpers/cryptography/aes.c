...
int mbedtls_aes_setkey_dec(
    ...
    if(mbedtls_aesni_has_support(MBEDTLS_AESNI_AES)) {
        mbedtls_aesni_inverse_key(
            (unsigned char*)RK, (const unsigned char*)(cty.buf + cty.rk_offset), ctx->nr);
        goto exit;
    }
#endif
...
static void aes_maybe_realign(mbedtls_aes_context* ctx) {
    ...
    if(new_offset != ctx->rk_offset) {
        memmove(
            ctx->buf + new_offset, // new address
...
#error "Unrecognised value for MBEDTLS_AESNI_HAVE_CODE"
     ...
     */
    {
        static const int num_tests = sizeof(aes_test_ecb_dec) / sizeof(*aes_test_ecb_dec);
     ...
     */
    {
        ...
        for(i = 0; i < num_tests << 1; i++) {
            ...
            if(verbose != 0) {
                mbedtls_printf(
                    "  AES-CTR-128 (%s): ", (mode == MBEDTLS_AES_DECRYPT) ? "dec" : "enc");
            }
